...
function SimpleModel:build(X, Y)
	...
	self.protos.pred = nn.Sequential():add(nn.Linear(self.opt.rnn_size, self.opt.input_size))
	...
	for name, proto in pairs(self.protos) do
		print('cloning '..name)
		self.clones[name] = mrnn.model_utils.clone_many_times(proto, self.opt.seq_length, not proto.parameters)
	end
	self.initstate_c = torch.zeros(self.opt.batch_size, self.opt.rnn_size)
